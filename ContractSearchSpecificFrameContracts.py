...
def filenameconsreportGridSpecific(grid, year):
    return (datetime.today()).strftime("%Y%m%d_%H%M%S") + "_" + SUBSTRING_CONS_REPORT + "_" + grid + "_" + str(
        year) + ".json"


def filenameprodreportGridSpecific(grid, year):
    return (datetime.today()).strftime("%Y%m%d_%H%M%S") + "_" + SUBSTRING_PROD_REPORT + "_" + grid + "_" + str(
        year) + ".json"


def getPathConsReport():
    return "\\\\energycorp.com\\common\\divsede\\Operations\\Schedules\\Germany\\ConsumptionReportsPython\\NeverUsedContractsReports\\"
...
def GiveOutput(url, filename, year):
    final_list = []
...
for grid in myDictGrids:
    ...
    for year in years:
        ...
        urlContractSearchCons = GetUrlContractSearchConsmasterSpecificYear(grid, year)

        # urlConsmaster = GetUrlContractSearchConsmaster(grid)

        filenameConsmaster = filenameconsreportGridSpecific(grid, year)
        filenameProdmaster = filenameprodreportGridSpecific(grid, year)

        GiveOutput(urlContractSearchCons, filenameConsmaster, year)
        GiveOutput(urlContractSearch, filenameProdmaster, year)
